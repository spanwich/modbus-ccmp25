...
The local git metadata is not readable from this checkout, so use concise imperative commit subjects such as `Fix watchdog SMC cadence` or `Add STM32MP25x VM DTS notes`. Keep each commit scoped to one behavior or investigation.
...
Treat watchdog, RIFSC, OP-TEE SMC, MMIO, and VM passthrough changes as high risk. Cross-check `docs/optee_smc_interfaces.md`, `docs/rifsc_dwmac_investigation.md`, and `docs/alignment_report_20260305.md` before editing those paths.
